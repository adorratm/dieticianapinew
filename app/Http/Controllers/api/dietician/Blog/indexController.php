...
namespace App\Http\Controllers\api\dietician\Blog;
...
class indexController extends Controller
{
    ...
    {
        $nutrients = Blog::where('writable_type',Dieticians::class)->where('writable_id',$this->user->id)->get();
        if ($nutrients) {
    ...
    { $validator = Validator::make($request->all(), [
        ...
        if ($validator->fails()) {
            ...
            $data['writable_type']=Dieticians::class;
    ...
    public function update($id, Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $post=Blog::findOrFail($id)->where('writable_id',$this->user->id)->first();
            ...
            return response()->json(["success" => true, "title" => "Başarılı!", "msg" => "Yazılarınız Başarıyla Güncellendi", "data" => new BlogResource($post)], 200, [], JSON_UNESCAPED_UNICODE);
        }
    ...
    {
        $blog=Blog::where('_id',$id)->where('writable_id',$this->user->id)->get();
        if ($blog->count()>0) {
            $title=$blog->first();
            $blog->first()->delete();
            return response()->json(["success" => true, "title" => "Başarılı!", "msg" => "Yazınız Başarıyla Silindi", "data" => new BlogResource($title)], 200, [], JSON_UNESCAPED_UNICODE);
        } else {
            return response()->json(["success" => false, "title" => "Başarısız!", "msg" => "Yazınız Silinirken Bir Hata İle Karşılaşıldı."], 200, [], JSON_UNESCAPED_UNICODE);
