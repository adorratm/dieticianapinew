...
namespace App\Http\Controllers\Api\Panel\edietfoods;
...
class indexController extends Controller
{
    ...
    {
        $nutrients = DB::table("edietfoods")->get();
        if ($nutrients) {
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $count = DB::table("edietfoods")->count();
            $data["rank"] = $count + 1;
            $data["isActive"] = 1;
            $data["slug"] = Str::slug($data["name"], "-");
            unset($data["vitaminName"]);
            unset($data["vitaminValue"]);
            $nutrients = DB::table("edietfoods")->insertGetId($data);
            foreach ($request->vitaminName as $key => $vitamin) {
                ...
                $add_data["isActive"] = 1;
                $add_data["edietfoods_id"] = (string)$nutrients;
                $add_data["rank"] = $key + 1;
                $nutrients_value = DB::table("edietfoods_value")->insert($add_data);
            }
            foreach ($request->criteriaName as $key => $criteria) {
                ...
                $add_data["edietfoods_id"] = (string)$nutrients;
                $add_data["rank"] = $key + 1;
                $nutrients_criteria = DB::table("edietfoods_criteria")->insert($add_data);
            }
    ...
    public function getFile($id)
    {
        ...
        if (!empty($id)) {
            $data = DB::table("edietfoods")->where("edietfoods_id", $id)->get();
            if (!empty($data)) {
    ...
    public function fileStore(Request $request, $id)
    {
        if (!empty($request->file())) {
            ...
            foreach ($request->file("file") as $key => $file):
                ...
                $path = $file->storeAs("uploads/edietfoods/{$strFileName}/", $fileNameWithExtension, "public");
                $count = DB::table("edietfoods_file")->where("edietfoods_id", $id)->count();
                $data["edietfoods_id"] = $id;
                $data["img_url"] = $path;
                $data["isActive"] = 1;
                $data["rank"] = $count + 1;
                $data["isCover"] = 0;
                $add = DB::table("edietfoods_file")->insert($data);
                if (!$path || !$add) {
                    $status = 0;
                }
            endforeach;
    ...
    {
        $nutrients = DB::table("edietfoods")
            ->where("_id", $id)->first();

        if ($nutrients) {

            $nutrients["criterias"] = DB::table("criteria")->where("isActive", 1)->get();
            $nutrients["images"] = DB::table("edietfoods_file")->where("edietfoods_id", $id)->get();
            $nutrients["values"] = DB::table("edietfoods_value")->where("edietfoods_id", $id)->get();
			 $nutrients["diseases"] = DB::table("diseases")->get();
			 $nutrients["meals"] = DB::table("meals")->get();
            $nutrients["criteria_values"] = DB::table("edietfoods_criteria")->where("edietfoods_id", $id)->get();
    ...
    public function update($id, Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data = $request->except("_token");
            $data["slug"] = Str::slug($data["name"], "-");
            $destroy = DB::table("edietfoods_value")->where("edietfoods_id", $id)->delete();
            foreach ($request->vitaminName as $key => $vitamin) {
                $add_data["title"] = $vitamin;
                $add_data["value"] = $request->vitaminValue[$key];
                $add_data["type"] = $request->vitaminType[$key];
                $add_data["isActive"] = 1;
                $add_data["edietfoods_id"] = $id;
                $add_data["rank"] = $key + 1;

                $nutrients_value = DB::table("edietfoods_value")->insert($add_data);
            }
            $destroy = DB::table("edietfoods_criteria")->where("edietfoods_id", $id)->delete();
            ...
            $data = DB::table("edietfoods")->where("_id", $id)->update($data);
    ...
    public function getBySearch(Request $request)
    {
        if (empty($request->search) || $request->search == "null") {
            return Redirect::to(route("panel.edietfoods.getAll", "table={$request->table}&per_page={$request->per_page}"));
        }
