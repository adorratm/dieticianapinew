...
class indexController extends Controller
{
    ...
    {
        $criteria = DB::table("recipe_categories")->get();
        if ($criteria) {
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $criteria=DB::table("recipe_categories")->insert($data);
    ...
    public function getFile($id)
    {
        ...
        if (!empty($id)) {
            $data = DB::table("recipe_categories")->where("recipe_id", $id)->get();
            if (!empty($data)) {
    ...
    public function fileStore(Request $request, $id)
    {
        if (!empty($request->file())) {
            ...
            foreach ($request->file("file") as $key => $file):
                ...
                $count = DB::table("recipes_file")->where("recipe_id", $id)->count();
                $data["recipe_id"] = $id;
                $data["img_url"] = $path;
                $data["isActive"] = 1;
                $data["rank"] = $count + 1;
    ...
    {
        $criteria = DB::table("recipe_categories")
            ->where("_id", $id)->first();

        if ($criteria) {

            $criteria["images"] = DB::table("recipes_file")->where("recipe_id", $id)->get();
            $criteria["values"] = DB::table("criteria_value")->where("recipe_id", $id)->get();
    ...
    public function getBySearch(Request $request)
    {
        ...
        foreach ($request->search_columns as $column) {
			$response->orwhere($column,"like","%". strtolower($request->search)."%")
